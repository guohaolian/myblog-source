...
> [!CAUTION]
>
> root用户默认的是localhost，说明只允许从本地登录mysql服务。而我们要从远程以root用户连接数据库，就必须修改host的值，改为**‘%’**：允许任何ip访问。
...
> [!CAUTION]
>
> 此命令可以授权任何在mysql数据库user表中的用户以远程登录的方式访问数据库，本例中以’root’作为举例，若想授权其他用户，只需修改’root’的值为指定用户即可，'root_pwd’为’root’用户对应的登录密码，可以修改为你想要授权用户的登录密码。
